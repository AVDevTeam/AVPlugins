...
IPv4Address* IPv4Address::parse(const std::string& value) {
	...
	if (value == "localhost") {
	...
	else if (value == "$HOME_NET") {
		octet[0] = 127;
		octet[1] = 0;
		octet[2] = 0;
		octet[3] = 1;
	}
	else if (value == "$EXTERNAL_NET") {
		octet[0] = 0;
		octet[1] = 0;
		octet[2] = 0;
		octet[3] = 0;
		...
		int l;
		int r = sscanf_s(value.c_str(), "%d.%d.%d.%d%n", octet, octet + 1, octet + 2, octet + 3, &l);
...
IPv6Address::IPv6Address(wide_integer::generic_template::uint128_t address) : IPAddress(), value(address) {
}
...
IPv6Address IPv6Address::operator - (int delta) const {
	return IPv6Address(value - (wide_integer::generic_template::uint128_t)delta);
}
